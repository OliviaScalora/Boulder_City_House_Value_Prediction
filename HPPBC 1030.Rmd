...
## **I. Introduction**
Boulder county is located in state of Colorado and has an estimated population of Boulder County as of 2019 was just over 326,000 according to Boulder county website. The county houses 740 miles of Rocky Mountains and westerns plains and is surrounded with mountains in the west, plains on the east and natural trough that runs between the plains and foothills. With visionary open space, land use and sustainability policies to forward-thinking and compassionate human services programs. Boulder is known for its public schools, safe neighborhoods make its housing maker desirable. The county has one of the best and most stable housing markets but only 48% of its residents own their houses. Even with contrasting property tax at 0.61%, and median income higher than the national average, the cost of living is higher than most US cities and its considered on of the most desirable places to live in. How then, with this information, can we predict the housing market? This analysis pulls relevant open source data from Boulder's Open Data portal and Tigris to produce a useful, predictive Ordinary Least Squares (OLS) regression model. 

```{r setup, message=FALSE, warning=FALSE}
#Setup- load necessary libraries and functions
...
## **II. Data**
To start off, we collect and trim the data from various sources to best suit our projections. From the Census American Community Survey, we download demographic data at the tract level. We pull median income, median rent, population living bellow poverty line, percent married householders, population count with a bachelors degree, aggregate travel time to work per tracts, and percent white population. Along with the census features, we will be using proximity to trailheads, business, commercial, historical wildfire data, lakes and reservoirs, areas of flooding etc to understand the correlation between sales price of housing and these features. We expect both positive and negative correlations.
We will be using this data to rank or appropriate the cost of housing depending on its proximity to these features. For example, a house close to a trailhead indicates proximity to recreation an we would expect higher house value as opposed to a house which is in a flood zone.
...
```
##### **Feature Engineering**
We must engineer our features to make them strong variables in our predictive model. We first count the number of trailheads within a 1/2 mile buffer of each home sale. We next use the nearest neighbor function to calculate average distance between a home sales 5 nearest neighbors to respective features i.e. historical wildfire sites, historical geological hazard sites, flood zones, lakes and reservoirs, and types of zoning such as agricultural, commercial and industrial. A lower average indicates closer proximity to these features.
...
```{r Data.Description.Table, message=FALSE, warning=FALSE}
...
ggcorrplot(
  ...
  type="lower",
  insig = "blank") +  
  labs(title = "Sale Price Correlation with Numeric Variables", caption = "figure 1") +
  plotTheme()
```
...
knownSales$LNTotalSF <- log(knownSales$TotalFinishedSF+1)
knownSales$LNAg <- log(knownSales$Ag.nn+1)
knownSales$LNIndustry <- log(knownSales$Industrial.nn+1)

```

```{r hists, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 6}
#plot log transformed histograms
histlake <- histogram( ~ lakes.nn +LNlakes.nn,layout=c(2,1),data = knownSales,
                      main='Nearest Neighbor to Lakes', xlab='Original Data (left) log transformed data (right)', col="#B3CDE3", breaks = 25, scales='free')

histSF <- histogram( ~ TotalFinishedSF +LNTotalSF,layout=c(2,1),data = knownSales,
                      main='Total Square Footage', xlab='Original Data (left) log transformed data (right)', col="#B3CDE3", breaks = 25, scales='free')

histAG <- histogram( ~ Ag.nn +LNAg,layout=c(2,1),data = knownSales,
                      main='Nearest Neighbor to Agricultural Zoning', xlab='Original Data (left) log transformed data (right)', sub= 'figure 2', col="#B3CDE3", breaks = 25, scales='free')

histI <- histogram( ~ Industrial.nn +LNIndustry,layout=c(2,1),data = knownSales,
                      main='Nearest Neighbor to Industrial Zoning', xlab='Original Data (left) log transformed data (right)', col="#B3CDE3", breaks = 25, scales='free')

grid.arrange(histlake, histSF, histAG, histI, ncol=2, nrow=2)
```
...
As expected closer to work higher the sale price. from figure 3.1 we can gather that the highest sale prices are in the tracts which are around 15 mins from work place.

percent of white population is positively correlated to the sales price, it should also considered that given Boulder county ebing predominantly white, will not have a strong contributor  to predicting the sales prices.

```{r scater.output, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
...
```{r maps, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 36}
...
reg <- lm(price ~ MedRent + HouseAge + PctMarried + BelPov100 + wildfire.nn + geohazard.nn + LNlakes.nn + floodzone.nn + Ag.nn + Business.nn + Industrial.nn + NumTrailHeads + bsmtSF + HeatingDscr + TotalBach + MedHHInc + TravelTime + nbrBedRoom + qualityCodeDscr + TotalFinishedSF + designCodeDscr, data = knownSales_nog)

summary(reg)
```

```{r regression.output, eval=FALSE, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
...
```{r inTrain, results='hide', message=FALSE, warning=FALSE}
...
#training regression
reg.training <-lm(price ~ MedRent+ HouseAge+ PctMarried+BelPov100+wildfire.nn+geohazard.nn
...
```{r test.reg, results='hide', message=FALSE, warning=FALSE}
#test regression on boulder.test
boulder.test <-
  boulder.test %>%
  mutate(SalePrice.Predict = predict(reg.training, boulder.test),
...
reg.cv <- 
  train(price ~ MedRent+ HouseAge+ PctMarried+BelPov100+wildfire.nn+geohazard.nn
        +LNlakes.nn+floodzone.nn+Ag.nn+Business.nn+Industrial.nn+NumTrailHeads+bsmtSF+HeatingDscr+TotalBach+
          MedHHInc+TravelTime+nbrBedRoom+qualityCodeDscr+
...
```{r spatial.clust.plot, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
...
```{r morans.I, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
#interpret
...
```{r test.output, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
...
##add neighborhood effects to plot
```
